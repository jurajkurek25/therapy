...
import { isLoggedIn, getUser, saveUser, getTransactions, saveTransactions, type User } from '@/lib/store';
...
export default function TopupPage() {
  ...
  const handlePurchase = async () => {
    ...
    const tx = {
      ...
      date: today,
      description: `Dokup kreditov · ${amount} kr.`,
      delta: amount,
      balance: newCredits,
      type: 'topup' as const,
    };
        ...
        <main className="main-content">
          ...
          {success ? (
            <div style={{ maxWidth: 500 }}>
              <div style={{ padding: 40, background: 'var(--ink)', color: 'var(--bg)', textAlign: 'center' }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--accent)', marginBottom: 16 }}>
                  ✓ Platba úspešná
                </div>
                <div style={{ fontFamily: 'var(--serif)', fontSize: 72, lineHeight: 1, letterSpacing: '-0.02em', marginBottom: 8 }}>
                  +{amount}
                </div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 12, opacity: 0.6, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 24 }}>
                  kreditov pridaných
                </div>
                <div style={{ fontFamily: 'var(--serif)', fontSize: 32 }}>
                  Nový zostatok: {user.credits} kreditov
                </div>
              </div>
              <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
                <button onClick={() => setSuccess(false)} className="btn btn-ghost" style={{ borderRadius: 0 }}>
                  Dokúpiť viac
                </button>
                <a href="/poukazky" className="btn btn-primary" style={{ borderRadius: 0, flex: 1, justifyContent: 'center' }}>
                  Generovať kód → {user.credits} kr.
                </a>
              </div>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 48 }}>
              <div>
                ...
                <button onClick={handlePurchase} disabled={paying} className="btn btn-primary"
                  ...
                  {paying ? 'Spracovávam platbu…' : `Zaplatiť ${fmt(amount)} € →`}
                </button>
              ...
              <div>
                <div style={{ background: 'var(--ink)', color: 'var(--bg)', padding: 28 }}>
                  ...
                  <div style={{ fontSize: 13, opacity: 0.7, lineHeight: 1.55, marginBottom: 12 }}>
                    Sedenie u Hedepy, Ksebe alebo Mojra stojí typicky <strong style={{ color: 'var(--bg)' }}>50–70 kreditov</strong> / 50 min.
                  </div>
                ...
                <div style={{ padding: 20, border: '1px solid var(--rule)', borderTop: 'none', background: 'var(--paper)', fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--ink)', display: 'block', marginBottom: 6 }}>Prečo bez servisného poplatku?</strong>
                  Za dokup kreditov si neúčtujeme 2,90 €. Ten platíte len raz mesačne v rámci predplatného. Dokup je čistá platba 1 € = 1 kredit.
                </div>

                <div style={{ padding: 20, border: '1px solid var(--rule)', borderTop: 'none', background: 'var(--paper)', fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--ink)', display: 'block', marginBottom: 6 }}>Platobné metódy</strong>
                  Platobná karta (VISA, Mastercard, Maestro), Apple Pay, Google Pay. Všetky platby sú spracované cez Stripe — vaše kartové údaje neukladáme.
                </div>
              </div>
            </div>
          )}

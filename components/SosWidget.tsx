...
export default function SosWidget({ user }: { user: UserLike }) {
  const eligible = user.memberMonths >= SOS_VESTING && user.sosDebt === 0;
  const hasDebt = user.sosDebt > 0;
  const monthsLeft = Math.max(0, SOS_VESTING - user.memberMonths);

  if (!eligible && !hasDebt) {
    const pct = Math.min(100, (user.memberMonths / SOS_VESTING) * 100);
    return (
      <div style={{ border: '1px solid var(--rule-2)', padding: 24, background: 'var(--paper)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: 28, color: 'var(--rule-2)', lineHeight: 1, flexShrink: 0 }}>◈</div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          <span>{user.memberMonths} / {SOS_VESTING} mesiace lojality</span>
          <span>{Math.round(pct)} %</span>
        </div>
        <p style={{ fontSize: 13, color: 'var(--muted)', margin: '14px 0 0', lineHeight: 1.55 }}>
          Po {SOS_VESTING} mesiacoch platenia vám odomkneme možnosť ísť do záporného zostatku o jedno sedenie.
        </p>
      </div>
    );
  }
  ...
  if (hasDebt) {
    ...
    return (
      <div style={{ border: '1px solid var(--rule-2)', padding: 24, background: 'var(--paper)', borderLeft: '3px solid var(--accent-ink)' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--muted)', margin: '14px 0 0', lineHeight: 1.55 }}>
          Dlh sa automaticky spláca z mesačných kreditov predplatného.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ border: '1px solid var(--ink)', padding: 24, background: 'var(--ink)', color: 'var(--bg)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 12 }}>
        ...
        <div style={{ fontFamily: 'var(--serif)', fontSize: 40, color: 'var(--accent)', lineHeight: 1, flexShrink: 0 }}>±{SOS_LIMIT}</div>
      </div>
